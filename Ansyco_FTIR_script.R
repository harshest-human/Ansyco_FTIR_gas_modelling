...
FTIR_input$Pipe[as.numeric(FTIR_input$Messstelle)>=7]  = "Pipe I"
FTIR_input$Pipe[as.numeric(FTIR_input$Messstelle)<=6]  = "Pipe II" 
...
CO2xheightxSW <- ggplot(FTIR_south_west, aes(x=as.factor(height), y=CO2, fill=(as.factor(Pipe))))+ 
...
CH4xheightxSW <- ggplot(FTIR_south_west, aes(x=as.factor(height), y=CH4, fill=(as.factor(Pipe))))+ 
        ggtitle("CH4 at varying heights (South-Southwest)")+
        xlab("Height (m)") + ylab("CH4 (ppm)")+ labs(fill = "Sampling_Line")+
        geom_boxplot()+ stat_compare_means(method = "anova")

NH3xheightxSW <- ggplot(FTIR_south_west, aes(x=as.factor(height), y=NH3, fill=(as.factor(Pipe))))+ 
...
CO2xheightxNE <- ggplot(FTIR_north_east, aes(x=as.factor(height), y=CO2, fill=(as.factor(Pipe))))+ 
        ggtitle("CO2 at varying heights (North-Northeast)")+
        xlab("Height (m)") + ylab("CO2 (ppm)")+ labs(fill = "Sampling_Line")+
        geom_boxplot()+ stat_compare_means(method = "anova")

CH4xheightxNE <- ggplot(FTIR_north_east, aes(x=as.factor(height), y=CH4, fill=(as.factor(Pipe))))+ 
        ggtitle("CH4 at varying heights (North-Northeast)")+
        xlab("Height (m)") + ylab("CH4 (ppm)")+ labs(fill = "Sampling_Line")+
        geom_boxplot()+ stat_compare_means(method = "anova")

NH3xheightxNE <- ggplot(FTIR_north_east, aes(x=as.factor(height), y=NH3, fill=(as.factor(Pipe))))+ 
